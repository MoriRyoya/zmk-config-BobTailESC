...
static const struct device *ball_action_sensor_dev;

static void invoke_ball_action(const struct device *dev, int idx) {
    const struct pixart_config *config = dev->config;
    if (ball_action_idx < 0 || idx < 0) {
        return;
    }
    const struct ball_action_cfg action_cfg = *config->ball_actions[ball_action_idx];
    if (idx >= (int)action_cfg.bindings_len) {
        return;
    }
...
static void ball_action_cancel(struct pixart_data *data);

static void ball_action_commit_work_fn(struct k_work *work) {
    ARG_UNUSED(work);
    ...
    struct pixart_data *data = dev->data;
    /* Key-up is on the left half; if the layer is already gone, this was
     * pointing, not a flick. Drop it. */
    if (get_input_mode_for_current_layer(dev) != BALL_ACTION) {
        ball_action_cancel(data);
        return;
    }
    int idx = data->ball_action_pending_idx;
    data->ball_action_pending_idx = -1;
    data->ball_action_delta_x = 0;
    data->ball_action_delta_y = 0;
    invoke_ball_action(dev, idx);
}

static K_WORK_DEFINE(ball_action_commit_work, ball_action_commit_work_fn);

static void ball_action_commit(struct k_timer *timer) {
    ARG_UNUSED(timer);
    k_work_submit(&ball_action_commit_work);
}

K_TIMER_DEFINE(ball_action_commit_timer, ball_action_commit, NULL);

static void ball_action_cancel(struct pixart_data *data) {
    k_timer_stop(&ball_action_commit_timer);
    data->ball_action_pending_idx = -1;
    data->ball_action_delta_x = 0;
    data->ball_action_delta_y = 0;
}
    ...
    if (x != 0 || y != 0) {
        if (input_mode == MOVE || input_mode == SNIPE) {
            ...
            if (ball_action_idx != -1) {
                ...
                if (idx != -1) {
                    data->ball_action_pending_idx = idx;
                    k_timer_start(&ball_action_commit_timer,
                                  K_MSEC(CONFIG_PMW3610_BALL_ACTION_SETTLE_MS), K_NO_WAIT);
                }

                /* Keep deferring while the ball is still moving. Firing on the
                 * way down would turn "release gesture, then point" into a
                 * leftover Mission Control. */
                if (data->ball_action_pending_idx >= 0 &&
                    (abs(x) + abs(y) > CONFIG_PMW3610_MOVEMENT_THRESHOLD)) {
                    k_timer_start(&ball_action_commit_timer,
                                  K_MSEC(CONFIG_PMW3610_BALL_ACTION_SETTLE_MS), K_NO_WAIT);
                }
